...
plot_grid(plotlist = l.plot, nrow =5, labels = n.max, label_colour = "red", label_x = .1)
```

export data for contingency analysis
```{r}
d.write <- left_join(d.compare, sum.gene)%>%
  select(strain_ID =title, gene=locus_tag, gene_name, position, snp_type, type, prevalence=avg.mut.n)%>%
  # add metadata
