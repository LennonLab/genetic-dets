...
#visualize
d.gene %>%
  mutate(gene_name = str_trunc(gene_name,30)%>%fct_infreq()%>%fct_rev())%>%
...
d.compare%>%
  ...
  filter(gene_name != "RNA_48")%>%
  filter(gene_name != "RNA_19")%>%
  filter(gene_name != "SynWH7803_2512")%>%
  filter(gene_name != "SynWH7803_2356")%>%
